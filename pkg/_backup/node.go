...
type Handler func(Flags) error

// handler will be called in Node.Parse, the param fv is the flags' name-value map
...
// func (n *Node) AddFlag(name, description string, hasValue bool) {
// 	newFlag := &Flag{
// 		Name:        name,
// 		Description: description,
// 		HasValue:    hasValue,
...
// 	// register into FlagSet
// 	n.FS.Func(name, description, func(s string) error {
// 		// Each time the flag is seen, fn is called with the value of the flag,
// 		// except the flag with no value.
// 		newFlag.Value = s
// 		newFlag.IsSet = true
// 		return nil
// 	})
// 	n.Flags[name] = newFlag
// }

func (n *Node) FlagBool(name string, value bool, usage string) *bool {
	return n.FS.Bool(name, value, usage)
}
...
func (n *Node) Print() {
	DFS(n, 0, func(n *Node, depth int) {
		...
		// print flags
		if n.Flags != nil && len(n.Flags) > 0 {
			fmt.Print(" {")
			i := 0
			for name, flag := range n.Flags {
...
func (n *Node) Parse(input string) error {
	...
	if err := targetNode.FS.Parse(flagStr); err != nil {
		if strings.HasPrefix(err.Error(), "flag needs an argument:") { // flag with no value
			var flagName string
			fmt.Sscanf(err.Error(), "flag needs an argument: -%s", &flagName)
			if targetNode.Flags[flagName].HasValue {
				return err
			} else { // special case: flag's HasValue is false
				targetNode.Flags[flagName].IsSet = true
			}
		} else {
			return err
		}
	...
	// check parsed flags
	for _, flag := range targetNode.Flags {
	...
	if targetNode.Handler != nil {
		err = targetNode.Handler(targetNode.Flags)
	} else {
		err = nil
	}

	// refresh flag's state for next parse
	for _, flag := range targetNode.Flags {
		flag.Refresh()
	}
	return err
...
func (n *Node) Usage() {
	...
	// TODO !!!!!! like root node engine, it's FS is init to nil, will cause error
	// NOTICE same error!!!!
	n.FS.PrintDefaults()
}
