// From telefraf/internal/rotate/file_writer.go
package rotate
...
// Implement io.Writer interface's Write method
...
func (w *FileWriter) rotateIfNeeded() error {
	if (w.interval > 0 && time.Now().After(w.expireTime)) ||
        ...
        fmt.Printf("test bytesWritten%v maxSizeInBytes%v\n", w.bytesWritten, w.maxSizeInBytes)
		if err := w.rotate(); err != nil {
...
func (w *FileWriter) rotate() (err error) {
	...
	now := time.Now()
    // Notice: If log mutiple times within the granularity set below (like sec, micro sec or milli sec)
    // and each of them triggeres the rotation, which will result rotatedFilename always staying the same,
    // so the file will be repeatedly covered
	rotatedFilename := fmt.Sprintf(w.filenameRotationTemplate, now.Format(DateFormat),
            strconv.FormatInt(now.Unix(), 10))
    fmt.Printf("test rename from %v to %v\n", w.filename, rotatedFilename)
